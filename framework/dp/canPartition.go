...
func canPartition(nums []int) bool {
	...
	for i := 1; i <= n; i++ {
		for j := 1; j <= sum; j++ {
			if j-nums[i-1] < 0 { // 装不下, 结果不变(注意这里，装不下等于之前的值，可能剩余容量为0，那么之前的就是 true，不是 0，那之前也装不满为 false)
				dp[i][j] = dp[i-1][j]
			} else { // 装或者不装背包的选择，只要有一个分支是 true 就满足条件(之前已经装满了，或者容量 j-nums[i-1] 时装满了（此时加上当前会装满）)
				dp[i][j] = dp[i-1][j] || dp[i-1][j-nums[i-1]] // 注意为什么是 dp[i-1]，因为物品只能选择一次，dp[i][xxx] 代表当前物品的不同容量的结果，
				// 而这里是要求放入当前物品之前，所能达到 j-wt[i-1] 的最大价值
			}
		}
	}
	// for i := 0; i <= n; i++ {
...
func canPartition2(nums []int) bool {
	...
	for i := 1; i <= n; i++ {
		...
		// 其实就是多次使用同一个 i 的结果（凑零钱里一个硬币可重复选）
		// 这个场景显然不行，因为每个物品放入后就已经不可以再次选择，需要选择 i-1 那个物品求得结果才行，本物品 （i） 的计算结果不能重复用
		// 在内层循环里先破坏了 dp[j-nums[i-1]] 的求解 dp[j-nums[i-1]] 取的不是 i-1 的那个结果而是
		// i 层循环求 dp[j] 计算环覆盖了的结果（我要要求的是 dp[i-1][j-nums[i-1]] 而不是 dp[i][j-nums[i-1]]）
		for j := sum; j >= 0; j-- {
			if j-nums[i-1] >= 0 {
