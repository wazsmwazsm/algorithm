...
func isMatch2(text, pattern string) bool {
	...
	return firstMatch && isMatch2(text[1:], pattern[1:])
}
...
func isMatch3(text, pattern string) bool {
	...
	if len(pattern) >= 2 && pattern[1] == '*' {
		...
		// 当前字符和模式没匹配上，只能选择 0 次匹配，isMatch3(text, pattern[2:])，text 不变，pattern 跳过 * 继续匹配
		// 当前字符和模式匹配上时，说明可以试着让 * 继续匹配，text 往下个字符移动，看是否还能匹配 *
		return isMatch3(text, pattern[2:]) || firstMatch && isMatch3(text[1:], pattern)
	}
...
// 使用备忘录剪枝
func isMatchDP(memo *map[string]bool, text, pattern string) bool {
	...
	firstMatch := text != "" && (pattern[0] == text[0] || pattern[0] == '.')
	if len(pattern) >= 2 && pattern[1] == '*' {
		(*memo)[text+":"+pattern] = isMatchDP(memo, text, pattern[2:]) || firstMatch && isMatchDP(memo, text[1:], pattern)
		return (*memo)[text+":"+pattern]
	}

	(*memo)[text+":"+pattern] = firstMatch && isMatchDP(memo, text[1:], pattern[1:])
	return (*memo)[text+":"+pattern]
}
...
func isMatch4(text, pattern string) bool {
	...
	return isMatchDP(&memo, text, pattern)
}
